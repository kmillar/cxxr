...
SEXP attribute_hidden R_do_data_class(/*const*/ Expression* call, const BuiltInFunction* op, Environment* env, const ArgList& args)
{
  if(op->variant() == 1) { // .cache_class() - typically re-defining existing cache
      call->check1arg("class");
      SEXP klass = args[0].value();
      ...
      return cache_class(class_str, args[1].value());
  }
  // class():
  call->check1arg("x");
  return R_data_class(args[0].value(), FALSE);
}
